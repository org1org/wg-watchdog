...
После приветствия установщик спрашивает, нужно ли продолжить: `[Y/n]`. Нажатие Enter означает `yes`. Необязательные возможности задаются как `[y/N]`, где Enter означает `no`. Информационные этапы выделяются зелёным цветом и отделяются пустыми строками от вывода системных команд. Для отключения цвета можно запустить установщик с переменной `NO_COLOR=1`.
...
Установленная программа занимает около 45 КБ без учёта конфигураций. Тестовый стенд находится только в репозитории и на роутер не устанавливается.
