...
- `/opt/var/lib/wg-watchdog/WireguardN.state` — состояние и история последней проверки;
- `/opt/etc/crontab` — автоматически управляемые строки запуска;
- `/opt/etc/crontab.wg-watchdog.bak` — резервная копия crontab;
- `/opt/var/run/wg-watchdog-WireguardN.lock` — временная блокировка параллельного запуска.
...
rm -rf /opt/etc/wg-watchdog.d /opt/var/lib/wg-watchdog
