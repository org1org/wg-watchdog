...
- выделяет включённые задания зелёным, выключенные красным и нумерует их с точкой;
...
- проверяет строгий манифест выпуска и обновляется из меню с автоматическим откатом;
...
Вверху показаны все найденные интерфейсы. Включённая проверка отмечается зелёным,
выключенная — красным, ненастроенный интерфейс — серым. При выборе изменения,
включения, ручной проверки, статуса или удаления менеджер заново показывает все
настроенные задания. Задание по Enter не выбирается; `0` возвращает в главное меню.
В небольшом окне длинный вывод разбивается на страницы. Размер окна учитывается
...
Установленная программа занимает около 75 КБ без учёта конфигураций. Компактный установщик,
тестовый стенд и изображение интерфейса на роутер не устанавливаются.

При запуске менеджер скачивает в `/tmp` только небольшой текстовый манифест
`RELEASE`. В нём указаны версия, неизменяемый commit и SHA-256 обоих скриптов.
Если найдена новая версия, вверху появляется заметное цветное уведомление.

После подтверждения оба файла сначала полностью загружаются в RAM. Менеджер
проверяет их размер, синтаксис, версию и SHA-256, а также свободное место. Только
после этого на `/opt` временно сохраняются старая и новая пары файлов. Замена
выполняется переименованиями внутри одной файловой системы; проверки cron на это
время приостанавливаются. Ошибка второй замены возвращает оба прежних файла.
Если питание пропало во время операции, следующий запуск `wgwm` завершит очистку
либо восстановит старую пару по служебному маркеру.

Постоянного кеша обновлений нет: временный каталог на `/opt` создаётся только на
время реальной установки и затем удаляется. Для обновления требуется примерно
размер старой и новой пар файлов плюс резерв 64 КБ (для текущей версии — около
220 КБ свободного места). Обычные проверки туннеля ничего в этот каталог не пишут.
...
Начиная с версии 1.6.0 запустите `wgwm` и выберите пункт проверки обновлений.
Для безопасного перехода с любой версии до 1.6.0 один раз выполните принудительную
установку из раздела выше. Старый файл `VERSION` намеренно оставлен на 1.5.4:
это не даёт менеджеру 1.5.4 применить новый выпуск прежним последовательным
механизмом без отката. После перехода используется манифест `RELEASE`.
...
В каталоге `tests` находится стенд с подменёнными системными командами. Он выполняет
65 проверок: восстановление туннеля, диалоги, установку, удаление, блокировки,
сохранность cron, временные файлы, целостность выпуска и аварийный откат:
